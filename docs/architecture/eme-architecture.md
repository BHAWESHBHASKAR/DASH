...
- Ingestion transport now supports batch writes (`POST /v1/ingest/batch`) with strict atomic rollback semantics, durable batch commit metadata records in WAL, and `commit_id` idempotent replay/conflict handling.
