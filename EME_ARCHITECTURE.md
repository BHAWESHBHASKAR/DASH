...
EME is a retrieval system designed for RAG where the primary data primitive is an **atomic claim with provenance**, not a standalone embedding vector.  
...
EME addresses these by storing a verifiable evidence graph and running query-time evidence computation.
